...
# Script adaped from https://github.com/FlowFun/M1-MRD-en-13C/blob/main/Legacy%2010C/Normalization%20DIAG%2010C%20navios.R by Benoit DUPONT and https://gist.github.com/yannabraham/c1f9de9b23fb94105ca5
...
TAG.channel <- 250000 #DIAG=250000 // FU=500000 // NBM=750000
final.cell.number <- 30000 #DIAG=30000 // FU=500000 // NBM=50000
...
lympho.pop <- point.in.polygon (point.x = new_logicle[,CD45.parameter.nb], 
                                ...
                                pol.y = coord$y, 
                                mode.checked = FALSE)
blast.pop <- point.in.polygon (point.x = new_logicle[,CD45.parameter.nb], 
                               point.y = new_logicle[,SSCINT.parameter.nb],
                               pol.x = coord.2$x, 
                               pol.y = coord.2$y, 
                               mode.checked = FALSE)
...
#Récupération des cellules blastiques
csv.logicle.corrected <- csv.logicle.corrected[blast.pop==1,]
